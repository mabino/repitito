# Repitito Copilot Instructions

- Favor C# with WPF (`net8.0-windows`) for UI changes. Keep business logic inside `Repitito/Core` and platform specific interactions under `Repitito/Services` so it stays testable.
...
- Update the custom console test runner in `Repitito.Tests/Program.cs` with new assertions when features change. Ensure it exits with a non-zero code on failure.
